...
class myenv():
    ...
    def step(self,state,action,show=False):
        ...
        Amt_=0
        if action==0:
            action=random.randint(1,2)#不允许本地
    ...
    def play(self,show=False):
        ...
        while not over:
            ...
            state = next_state
        save_agent(emtEmtlist,str(L)+'OemtEmtlist.pkl')
...
#     L=6400
    ...
    with torch.no_grad():
        save_agent(test_resultlist,str(L)+'Otest_resultlist.pkl')
        save_agent(Energylist,str(L)+'OEnergylist.pkl')
        save_agent(Utilitylist,str(L)+'OUtilitylist.pkl')
        save_agent(Latencylist,str(L)+'OLatencylist.pkl')
        save_agent(Privacylist,str(L)+'OPrivacylist.pkl')
        save_agent(losslist,str(L)+'Olosslist.pkl')
        save_agent(actionbar,str(L)+'Oactionbar.pkl')
        if 0:
