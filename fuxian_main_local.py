...
class myenv():
    ...
    def play(self,show=False):
        ...
        while not over:
            ...
            state = next_state
        save_agent(emtEmtlist,str(L)+'LemtEmtlist.pkl')
...
#     L=6400
    ...
    with torch.no_grad():
        save_agent(test_resultlist,str(L)+'Ltest_resultlist.pkl')
        save_agent(Energylist,str(L)+'LEnergylist.pkl')
        save_agent(Utilitylist,str(L)+'LUtilitylist.pkl')
        save_agent(Latencylist,str(L)+'LLatencylist.pkl')
        save_agent(Privacylist,str(L)+'LPrivacylist.pkl')
        save_agent(losslist,str(L)+'Llosslist.pkl')
        save_agent(actionbar,str(L)+'Lactionbar.pkl')
        if 0:
