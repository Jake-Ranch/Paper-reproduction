...
class myenv():
    ...
    def play(self,show=False):
        ...
        while not over:
            ...
            state = next_state
        save_agent(emtEmtlist,str(M)+'emtEmtlist.pkl')
...
#     L=6400
    ...
    with torch.no_grad():
        save_agent(test_resultlist,str(M)+'test_resultlist.pkl')
        save_agent(Energylist,str(M)+'Energylist.pkl')
        save_agent(Utilitylist,str(M)+'Utilitylist.pkl')
        save_agent(Latencylist,str(M)+'Latencylist.pkl')
        save_agent(Privacylist,str(M)+'Privacylist.pkl')
        save_agent(losslist,str(M)+'losslist.pkl')
        save_agent(actionbar,str(M)+'actionbar.pkl')
        if 0:
